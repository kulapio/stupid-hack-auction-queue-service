...
const ChaincodeAssetService = require('./service/chaincode-asset-service')
...
let proxyClientInstance = null;
const proxyUsername = "ProxyUser1";
const proxyPassword = "extraOrdinarySecurePassword";
...
app.use(async (req, res, next) => {
  ...
  if (proxyClientInstance) {
    fabricStarterClient = proxyClientInstance;
  }
  else {
    logger.debug('load Proxy Cert');
    ...
    proxyClientInstance = fabricStarterClient;
  }


  // Init chaincode service
  const channelId = 'common'
  const chaincodeId = 'aabp_asset'
  assetService = new ChaincodeAssetService(fabricStarterClient, channelId, chaincodeId)
...
const appRouter = (app) => {
  ...
  // Asset list
  app.get('/assetmanagement/asset/list', asyncMiddleware(async (req, res, next) => {
    let assets = await assetService.list(0, 10)
  ...
  // Insert asset
  app.post('/assetmanagement/asset', asyncMiddleware(async (req, res, next) => {
    let item = req.body
    let result = await assetService.insert(item)
  ...
  // Update asset
  app.patch('/assetmanagement/asset/:assetId', asyncMiddleware(async (req, res, next) => {
    const id = req.params.assetId
    const partialItemData = req.body
    let result = await assetService.update(id, partialItemData)
  ...
  // Delete asset
  app.delete('/assetmanagement/asset/:assetId', asyncMiddleware(async (req, res, next) => {
    const id = req.params.assetId
    let result = await assetService.delete(id)
